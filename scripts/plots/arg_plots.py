...
def main():
    ...
    for organism in genome_dir:
        arg_files = list_files(all_file_path,genomes_path,organism)
    print_status(arg_files)

    '''Building a dir of fna files'''
    arg_path = "{}/argannot_files".format(dirpath)
    create_genomes_dir(arg_path)
    os.chdir(arg_path)
    for file in arg_files:
        move_file(file, arg_path)

    '''Building metadata'''
    #All genes to each genome
    agr_metadata = {}
    arg = list_files_simple(arg_path)
    for f in arg:
        with open(f) as arg_info:
            parse_genes(f, arg_info, agr_metadata)
    #All genes that occured
    all_genes = sorted(set(get_all_genes(agr_metadata)))
    ...
    df_major_classes = build_class_df(df_info, all_arg_classes, agr_metadata)
    ...
    plt.show()
    full_plot.savefig("genome_arg.pdf", bbox_inches='tight')
    not_full.savefig("genome_arg_not_scalled.pdf", bbox_inches='tight')
