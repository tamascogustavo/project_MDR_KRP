...
def parse_fasta(file, name, dicta):
    new_name = "{}_contigs.fasta".format(name.split('/')[-2])
    all_seq = []
    for line in file:
        if line.startswith(">"):
            id = line.strip().split()
...
def lol(fasta):
    ...
    for line in fasta:
        if line.startswith(">"):
            ...
            test_fix[id_name] += line.strip()
    #print(test_fix.keys())
    #print(test_fix.values())
    values = [len(x) for x in test_fix.values()]
    #print([len(x) for x in test_fix.values()])
    print(sum(values))
...
def main():
    ...
    '''Plasmid'''
    genomes_path = "{}{}".format(path_to_all_info,directories[1])
    ...
    for file in contig_files:
        with open(file) as contig:
            print(file.split("/")[-2])
            lol(contig)
            

            #parse_fasta(contig, file, fasta_dict)
